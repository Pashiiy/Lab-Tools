/** Shared chart theme from `--lt-chart-*` tokens. */
export function getChartTheme() {
  const root = getComputedStyle(document.documentElement);
  const get = (token, fallback) => root.getPropertyValue(token).trim() || fallback;
...
export function chartTooltipStyle(theme) {
  return {
